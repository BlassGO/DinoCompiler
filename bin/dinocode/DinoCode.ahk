...
gui(do:="",onevent:="",reset:=false) {
   ...
   if do {
	   ...
	   load_config(0 . Chr(1) . labels[onevent],,,FD_CURRENT)
   }
   return
}
...
load_config(configstr:="",local:=false,local_obj:="",from_fd:=0,from_type:="",newmain:=false) {
   ...
   (newmain||!isObject(FD)) ? (last_label:="",FD_CURRENT:="",unexpected:="",main_nickname:="",Delimiter:=Chr(34),Escape:="\",script_section:={},GLOBAL:={},FD:={1:new DObj()},SIGNALME:={exit:1, def:{}},_escape:=["`a","`b","`f","`n","`r`n","`r","`t","`v"],_result:=[],_evaluated:=[],_thread:={},lang(,,,,,true),gui(,,true),config_rc())
